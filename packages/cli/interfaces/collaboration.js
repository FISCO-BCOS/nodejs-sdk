...
const {
    encodeParams
} = require('../../api/common/web3lib/web3sync');
    ...
    if (abi.type === 'struct') {
        ...
        components.forEach(component => {
            if (data[component.name] == 'undefined') {
                throw Error(`no field named ${component.name} in data: ${data}`);
            }
...
interfaces.push(produceSubCommandInfo({
    ...
    (argv) => {
        ...
        return getBlockHeight(config).then(blockHeight => {
            ...
            let signedTx = Web3Sync.signTransaction(postdata, privateKey, encryptType, null);
            return web3jService.sendRawTransaction(signedTx).then(result => {
                ...
                if (result.status === '0x16') {
                    ...
                    let buffer = Buffer.from(output.substring(2), 'hex');
                    return {
                        status: result.status,
...
interfaces.push(produceSubCommandInfo({
        ...
        args: [{
            ...
            },
            {
                ...
                options: {
                    type: 'string',
                    describe: 'The parameters(splitted by space) of the contracct',
                    flag: FLAGS.VARIADIC
                }
            }
        ]
    ...
    argv => {
        ...
        return cnsService.queryCnsByNameAndVersion(contractName, "collaboration").then(contractInfo => {
            if (contractInfo.length != 1) {
                throw new Error(`the contract is not exist: ${contractName}`);
            }

            contractInfo = contractInfo[0];
            let name = contractInfo.name;
            if (name != contractName) {
                throw new Error(`contract name is not match: the provided is "${contractName}" but the remote is ${name}`);
            }
            ...
            ctorABI.forEach((inputABI, index) => {
                ...
                encodedParams = Buffer.concat([encodedParams, encodedInput]);
            });
            let selector = hash(contractName, config.encryptType).slice(0, 8);
            ...
            return getBlockHeight(config).then(blockHeight => {
                ...
                return web3jService.sendRawTransaction(signedTx).then(result => {
                    if (result.status === '0x0') {
                        let buffer = Buffer.from(result.output.substring(2), 'hex');
                        let contractId = ScaleCodec.decodeU32(buffer).data;
                        return {
                            status: result.status,
                            contractId,
                            transactionHash: result.transactionHash
...
interfaces.push(produceSubCommandInfo({
        ...
        args: [{
            ...
            },
            {
                ...
                options: {
                    type: 'string',
                    describe: 'The name of the exercised contract',
                }
            },
            {
                name: 'contractId',
                options: {
                    type: 'string',
                    describe: 'The id of the exercised contract',
                }
            },
            {
                name: 'rightName',
    ...
    argv => {
        ...
        let contractName = argv.contractName;
        let contractId = parseInt(argv.contractId);
        if (isNaN(contractId)) {
        ...
        return cnsService.queryCnsByNameAndVersion(contractName, "collaboration").then(contractInfo => {
            ...
            let right = rights.findIndex(right => right.name == rightName);
            if (right === -1) {
            ...
            let selector = hash(`${contractName}(${rightName})`, config.encryptType).slice(0, 8);
            selector = Buffer.from(selector, 'hex');
            encodedParams = Buffer.concat([selector, encodedParams]);
            encodedParams = '0x' + encodedParams.toString('hex');
            ...
            return getBlockHeight(config).then(blockHeight => {
                ...
                return web3jService.sendRawTransaction(signedTx).then(result => {
                    if (result.status === '0x0') {
                        let data = Buffer.from(result.output.substring(2), 'hex');
                        let outputABIs = right.outputs;
                        let outputs = []
                        outputABIs.forEach(outputABI => {
                            let decode_result = decode(data, outputABI);
                            data = data.slice(decode_result.take);
...
interfaces.push(produceSubCommandInfo({
    ...
    argv => {
        ...
        return cnsService.queryCnsByNameAndVersion(contractName, "collaboration").then(contractInfo => {
            ...
            let selector = '0x' + hash(`$${contractName}`, config.encryptType).slice(0, 8);
            let encodedParams = selector + ScaleCodec.encodeU32(contractId).toString('hex');
            ...
            return getBlockHeight(config).then(blockHeight => {
                ...
                return web3jService.sendRawTransaction(signedTx).then(result => {
                    if (result.status === '0x0') {
                        let buffer = Buffer.from(result.output.substring(2), 'hex');
                        let decode_result = decode(buffer, outputABI);
                        return {
                            status: result.status,
