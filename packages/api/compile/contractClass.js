...
const { createMethodDecoder } = require("../decoder");
const { createFunctionDescription } = require("../common/utils");
...
function createCodeForGetAddress() {
    let code = createCodeForAddressCheck() + "return this.address;";
    return code;
}
        ...
        newInstance: () => {
            ...
            for (let i = 0; i < contractClass.abi.length; ++i) {
                ...
                switch (item.type) {
                    case "function": {
                        if (
                            contract._functionABIMapper.has(contract[item.name])
                        ) {
                            throw new Error("function override is not allowed");
                        }

                        let func = createFunctionDescription(item);
                        ...
                        if (item.constant) {
                            contract[item.name] = new Function(
                                parameters,
                                createCodeForConstantMethod(item.name)
                            );
                        } else {
                            contract[item.name] = new Function(
                                parameters,
                                createCodeForMutableMethod(item.name)
                            );
                        }
                    ...
                    case "constructor": {
                        ...
                        let parameters = item.inputs.map((input) => input.name);
                        contract.$deploy = new Function(
                            "web3jService," + parameters.join(","),
                            createCodeForConstructor()
                        );

                        let contractAbi = new ethers.utils.Interface(
                            contract.abi
                        );
                        ...
                        Object.defineProperty(contract.$deploy, "encodeABI", {
                            value: (bin, params = []) => {
                                if (inputs.length !== params.length) {
                                    throw new Error(
                                        `wrong number of parameters for constructor, expected ${inputs.length} but got ${params.length}`
                                    );
                                }
                                ...
                                if (params.length !== 0) {
                                    ...
                                    contractBin += encodedParams
                                        .toString("hex")
                                        .substr(2);
                                }

                                return contractBin.indexOf("0x") === 0
                                    ? contractBin
                                    : "0x" + contractBin;
                            },
                            writable: false,
            ...
            contract.$getEventABIOf = new Function(
                "name",
                createCodeForGetEventABIOf()
            );

            // `$getFunctionABIOf(name)`, get function ABI of the specified name, throw if the abi not exists
            contract.$getFunctionABIOf = new Function(
                "name",
                createCodeForGetFunctionABIOf()
            );
