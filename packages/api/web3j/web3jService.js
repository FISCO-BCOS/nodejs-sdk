...
const { selectNode, createFunctionDescription } = require("../common/utils");
...
class Web3jService extends ServiceBase {
    ...
    async getBlockNumber() {
        let promise = this._constructRequest("getBlockNumber", [
            this.config.groupID,
        ]);
    ...
    async getConsensusStatus() {
        return this._constructRequest("getConsensusStatus", [
            this.config.groupID,
        ]);
    }
    ...
    async getClientVersion() {
        return this._constructRequest("getClientVersion", [
            this.config.groupID,
        ]);
    }
    ...
    async getBlockByHash(blockHash, includeTransactions) {
        ...
        return this._constructRequest("getBlockByHash", [
            this.config.groupID,
            blockHash,
            includeTransactions,
        ]);
    ...
    async getBlockByNumber(blockNumber, includeTransactions) {
        ...
        return this._constructRequest("getBlockByNumber", [
            this.config.groupID,
            blockNumber,
            includeTransactions,
        ]);
    ...
    async getBlockHashByNumber(blockNumber) {
        ...
        return this._constructRequest("getBlockHashByNumber", [
            this.config.groupID,
            blockNumber,
        ]);
    }
    ...
    async getTransactionByHash(transactionHash) {
        check(arguments, Str);
        return this._constructRequest("getTransactionByHash", [
            this.config.groupID,
            transactionHash,
        ]);
    }
    ...
    async getTransactionByBlockHashAndIndex(blockHash, transactionIndex) {
        ...
        return this._constructRequest("getTransactionByBlockHashAndIndex", [
            this.config.groupID,
            blockHash,
            transactionIndex,
        ]);
    ...
    async getTransactionByBlockNumberAndIndex(blockNumber, transactionIndex) {
        ...
        return this._constructRequest("getTransactionByBlockNumberAndIndex", [
            this.config.groupID,
            blockNumber,
            transactionIndex,
        ]);
    ...
    async getPendingTransactions() {
        return this._constructRequest("getPendingTransactions", [
            this.config.groupID,
        ]);
    }

    async getPendingTxSize() {
        return this._constructRequest("getPendingTxSize", [
            this.config.groupID,
        ]);
    }

    async getTotalTransactionCount() {
        return this._constructRequest("getTotalTransactionCount", [
            this.config.groupID,
        ]);
    }
    ...
    async getTransactionReceipt(txHash) {
        check(arguments, Str);
        return this._constructRequest("getTransactionReceipt", [
            this.config.groupID,
            txHash,
        ]);
    }
    ...
    async getCode(address) {
        ...
        return this._constructRequest("getCode", [
            this.config.groupID,
            address,
        ]);
    }
    ...
    async getSystemConfigByKey(key) {
        check(arguments, Str);
        return this._constructRequest("getSystemConfigByKey", [
            this.config.groupID,
            key,
        ]);
    }
    ...
    _checkParameters(func, params) {
        ...
        if (inputs.length !== params.length) {
            throw new Error(
                `wrong number of arguments for \`${name}\`, expected ${inputs.length} but got ${params.length}`
            );
        }
    }
    ...
    async sendRawTransaction(...args) {
        if (args.length === 1) {
            ...
            func = createFunctionDescription(func);
            let blockNumber = await getBlockHeight(this.config);
            let signTx = this._rawTransaction(
                to,
                func,
                params,
                blockNumber + 500,
    ...
    async deploy(abi, bin, parameters, type, who = null) {
        ...
        if (inputs.length !== parameters.length) {
            throw new Error(
                `wrong number of parameters for constructor, expected ${inputs.length} but got ${parameters.length}`
            );
        }
    ...
    async call(to, func, params, who = null) {
        ...
        return this._constructRequest("call", [
            this.config.groupID,
            {
